var sel = document.getElementById("select1"),
		...
		function addr(){	
			...
			if (xhr != null) {
				xhr.addEventListener("load",function(){
					if (xhr.status == 200) {
						...
						for(var i=0,max=data.length;i<max;i++){		
								...
								newlitext=document.createTextNode(stitle);
				...
				xhr.open("get", "GetAttrac.jsp?cate1no="+sel.value+"&cate2no="+sel2.value+"&countyno="+sel3.value+"&distno="+sel4.value, true); //true表示同步
				xhr.send();
			} else {
				alert("請升級您的瀏覽器!!");
			}			
