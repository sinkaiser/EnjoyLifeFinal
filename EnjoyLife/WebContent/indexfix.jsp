...
<%@include file="/includes/header" %>	
	...
	<script>
		...
		(function($) {
			...
			$.getJSON("${pageContext.request.contextPath}/GetLittleJson", {
				"id" : 1
			}, function(data) {
				...
				if (time) {
					if ((new Date() - new Date(time)) > new Date(1000 * 60 * 60
							* 24 * 30 * 3)) {
						alert("超過");
						flag1 = true;
					}
				...
				function aa() {
					...
					if (rans[1] == 1) {
						...
						$('#xxx').attr(
								"src",
								"${pageContext.request.contextPath}/GetImg?imgid="
										+ imgid[1]);
											...
											.click(
													function() {
														...
														$
																.ajax({
																	"type" : "post",
																	"url" : "${pageContext.request.contextPath}/Little",
																	"dataType" : "text",
																	"data" : {
																		"do" : "insert",
																		"targetNo" : 4,
